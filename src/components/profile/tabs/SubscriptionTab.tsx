...
export default function SubscriptionTab({ credit, loading, error, refetch }: SubscriptionTabProps) {
  ...
  const planName =
    credit?.planName && credit.planName !== 'none' ? credit.planName : 'No active plan';
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-secondary mt-1">
            Credit balance, access flags, and recent credit activity
          </p>
        </div>
        <button
      ...
      <div
        className={`flex items-center gap-3 rounded-xl border p-4 ${active ? 'border-emerald-500/20 bg-emerald-500/5 text-emerald-400/90' : 'border-border-default bg-elevated text-text-secondary'}`}
      >
        <div
          className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border ${active ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-elevated text-text-secondary border-border-default'}`}
        >
          <Shield size={20} />
        </div>
        <div>
          <h4 className="text-sm font-bold text-text-primary">
            {active ? `${planName} Subscription Active` : 'Starter Account'}
          </h4>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-border-default bg-elevated p-4">
          ...
          <p className="mt-2 text-xl font-black text-text-primary font-mono">
            {(credit?.credits ?? 0).toLocaleString(undefined, {
        ...
        <div className="rounded-xl border border-border-default bg-elevated p-4">
          ...
          <p className="mt-2 text-xs font-bold text-text-primary">
            {formatDate(credit?.expiresAt ?? null)}
          </p>
        </div>
        <div className="rounded-xl border border-border-default bg-elevated p-4">
          <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-text-secondary">
          ...
          {ACCESS_FLAG_LABELS.map(({ key, label }) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[10px] font-bold uppercase tracking-wider ${enabled ? 'text-emerald-400' : 'text-text-muted'}`}
                >
          ...
          <div className="rounded-xl border border-border-default overflow-hidden max-h-80 overflow-y-auto scrollbar-none">
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 z-10 bg-elevated border-b border-border-default">
                <tr>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-text-secondary">
                    Amount
                  </th>
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-text-secondary">
                    Type
                  ...
                  <th className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-text-secondary text-right">
                    Date
                  </th>
                </tr>
              </thead>
                ...
                {logs.map((log) => {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-elevated transition-colors">
                      <td
                        className={`px-4 py-2 text-xs font-mono font-bold ${positive ? 'text-emerald-400' : log.amount < 0 ? 'text-rose-400' : 'text-text-secondary'}`}
                      >
                        {positive ? '+' : ''}
                      ...
                      <td className="px-4 py-2 text-xs text-text-primary max-w-[240px] truncate">
                        {log.description}
                      </td>
                      <td className="px-4 py-2 text-xs text-text-secondary text-right font-mono">
                        {formatCreditLogDate(log.createdAt)}
                      </td>
                    </tr>
                  );
                })}
