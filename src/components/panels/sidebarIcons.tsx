'use client';

/**
 * The two right-rail glyphs, inlined.
 *
 * They are `RiBrainAi3Line` (Remix Icon) and `MdLibraryBooks` (Material Icons),
 * the same art `react-icons` ships, copied out of its own icon data rather than
 * imported — because importing them BREAKS THE PRODUCTION BUILD.
 *
 * `react-icons` v5 publishes one barrel per set (`react-icons/md/index.mjs` and
 * friends) and no per-icon entry points, so `import { MdLibraryBooks } from
 * 'react-icons/md'` pulls the whole set into the module graph: ~4000 icons for
 * `md`, ~3000 for `ri`. The tree already imports `bs`, `io5` and `sl`, and adding
 * these two tipped turbopack over an internal limit:
 *
 *     thread 'tokio-runtime-worker' panicked at turbopack-ecmascript/src/lib.rs:2429
 *     The high bits of the position 2248738 are not all 0s or 1s.
 *     modules_header_width=10, module=620
 *
 * `modules_header_width=10` packs a module index into 10 bits, and the merged
 * client chunk for `app/page.tsx` overflowed it. `next build` survives it;
 * `next build --turbopack`, which is what `npm run build:web` and therefore the
 * Docker image use, does not — so this only failed on the droplet.
 *
 * Inlining keeps the glyphs the design asked for and keeps the barrels out. Both
 * upstream sets are Apache-2.0 (Remix Icon, Material Icons), so the path data
 * carries over cleanly.
 *
 * The `size` prop mirrors react-icons' own API (a number applied to width and
 * height, `currentColor` fill) so call sites read the same either way.
 */

import React from 'react';

interface SidebarIconProps {
  /** Applied to both width and height, as react-icons does. */
  size?: number | string;
  className?: string;
}

/** Remix Icon `brain-ai-3-line` — the Deep Quant agent. */
export function BrainAiIcon({ size = 24, className }: SidebarIconProps) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      width={size}
      height={size}
      className={className}
      aria-hidden="true"
      focusable="false"
    >
      <path d="M19.5 4.7832V7.6709L22 9.11426V14.8867L19.499 16.3311L19.5 19.2178L14.5 22.1045L12 20.6611L9.5 22.1045L4.5 19.2178V16.3311L2 14.8877L2.00098 9.11328L4.5 7.66992V4.78418L9.5 1.89746L11.999 3.34082L14.501 1.89648L19.5 4.7832ZM13 5.07227V7H11V5.07324L9.5 4.20703L6.49902 5.93848V8.8252L4 10.2676V13.7334L6.5 15.1768V18.0635L9.5 19.7959L11 18.9287V17H13V18.9297L14.5 19.7959L17.5 18.0625V15.1768L20 13.7324V10.2695L17.499 8.8252L17.5 5.9375L14.501 4.20605L13 5.07227ZM14.2646 13.1602C14.3529 12.9473 14.6472 12.9473 14.7354 13.1602L14.8623 13.4648C15.0783 13.986 15.4807 14.4027 15.9873 14.6279L16.3457 14.7871C16.5511 14.8784 16.5511 15.1773 16.3457 15.2686L15.9658 15.4375C15.4721 15.6571 15.0761 16.0586 14.8564 16.5625L14.7334 16.8447C14.6432 17.0517 14.3569 17.0517 14.2666 16.8447L14.1436 16.5625C13.9239 16.0586 13.5279 15.6571 13.0342 15.4375L12.6543 15.2686C12.4489 15.1773 12.4489 14.8784 12.6543 14.7871L13.0127 14.6279C13.5193 14.4027 13.9217 13.986 14.1377 13.4648L14.2646 13.1602ZM9.58789 7.7793C9.74239 7.40671 10.2577 7.4067 10.4121 7.7793L10.6338 8.31445C11.0118 9.22695 11.7161 9.95624 12.6025 10.3506L13.2305 10.6289C13.5899 10.7887 13.5897 11.3117 13.2305 11.4717L12.5654 11.7676C11.7013 12.152 11.0086 12.8548 10.624 13.7373L10.4082 14.2324C10.2504 14.5948 9.74973 14.5948 9.5918 14.2324L9.37598 13.7373C8.99143 12.8548 8.29875 12.152 7.43457 11.7676L6.76953 11.4717C6.41033 11.3117 6.41022 10.7887 6.76953 10.6289L7.39746 10.3506C8.2839 9.95624 8.98832 9.22697 9.36621 8.31445L9.58789 7.7793Z" />
    </svg>
  );
}

/** Material Icons `library_books` — the active workspace's panel. */
export function LibraryBooksIcon({ size = 24, className }: SidebarIconProps) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      width={size}
      height={size}
      className={className}
      aria-hidden="true"
      focusable="false"
    >
      <path fill="none" d="M0 0h24v24H0z" />
      <path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2m-1 9H9V9h10zm-4 4H9v-2h6zm4-8H9V5h10z" />
    </svg>
  );
}
