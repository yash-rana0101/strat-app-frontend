...
function applyQaFrame(session: QuantSession, payload: StreamEventPayload): QuantSession {
  ...
  switch (payload.event) {
    ...
    case 'TOOL_CALL_RESULT': {
      // The result payload can contain full candle sets and indicator objects. The
      // tool timeline communicates execution status only, matching the analysis UI;
      // the model's prose answer is where interpreted results belong.
      return session;
    }
    case 'TOOL_CALL_END': {
      const tool = typeof data?.tool === 'string' ? data.tool : '';
      ...
      const pendingIndex = activity.findLastIndex((line) => line === `> ${tool}…`);
      const completed = status === 'failure' ? `! ${tool}` : tool;
      if (pendingIndex === -1) activity.push(completed);
      else activity[pendingIndex] = completed;
      messages[index] = { ...turn, activity };
      break;
    }
    case 'RUN_FINISHED': {
