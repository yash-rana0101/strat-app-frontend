...
export function getThemeOverrides(
  ...
  // Colors sourced from globals.css — :root (dark, --bg-base #191a1b) and .light (--bg-base #f0eee9)
  const bg = isDark ? '#191a1b' : '#f0eee9';
  const grid = isDark ? '#242527' : '#ddd8ce';
  const text = isDark ? '#8a8f98' : '#4a5568';
  const scaleBg = isDark ? '#191a1b' : '#f0eee9';
  const up = '#10b981';
  const down = '#ef4444';
