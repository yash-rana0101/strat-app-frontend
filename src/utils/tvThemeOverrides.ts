...
export function getThemeOverrides(
  ...
  // Colors sourced from globals.css — :root (dark, --chart-bg #0d0d0e, --bg-surface #191a1b) and .light (--bg-base #fcfaf7)
  const bg = isDark ? '#0d0d0e' : '#fcfaf7';
  const grid = isDark ? '#1c1d1e' : '#f2ece2';
  const text = isDark ? '#8a8f98' : '#4a5568';
  const scaleBg = isDark ? '#0d0d0e' : '#fcfaf7';
  const up = '#10b981';
  const down = '#ef4444';
